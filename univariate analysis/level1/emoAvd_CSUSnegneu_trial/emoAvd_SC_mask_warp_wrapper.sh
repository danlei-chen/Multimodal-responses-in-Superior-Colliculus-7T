...
rsync $SCRIPTPATH/model/$PROJNAME/{emoAvd_SC_mask_warp_startup.sh,emoAvd_SC_mask_warp.py} /scratch/$USER/$PROJNAME/wrkdir/
chmod a+rwx /scratch/$USER/$PROJNAME/wrkdir/emoAvd_SC_mask_warp_startup.sh
cd /scratch/$USER

$SINGULARITY exec  \
--bind "$DATA/BIDS_modeled/$PROJNAME/nosmooth:/scratch/data" \
--bind "/autofs/cluster/iaslab_7T/FSMAP/FSMAP_data/BIDS_modeled/subject_SC_mask/emoAvd/dartel_flow:/scratch/dartel_flow" \
--bind "/scratch/$USER/$PROJNAME/output:/scratch/output" \
--bind "/scratch/$USER/$PROJNAME/wrkdir:/scratch/wrkdir" \
$IMAGE\
/scratch/wrkdir/emoAvd_SC_mask_warp_startup.sh
