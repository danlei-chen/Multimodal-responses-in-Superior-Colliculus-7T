...
# subj_list = glob.glob('/Users/chendanlei/Dropbox (Partners HealthCare)/U01/EmotionAvoidanceTask/BIDS_BehavioralData/sub-127/')
subj_list = glob.glob('/scratch/data/sub-*/func/')
for subj_dir in subj_list:
    # subj_dir = '/Users/chendanlei/Dropbox (Partners HealthCare)/U01/EmotionAvoidanceTask/BIDS_BehavioralData/sub-061/'
    
    #get valid runs only
    #run_list = pd.read_csv('/Users/chendanlei/Google Drive/U01/AffPainTask_connectivity/analysis/cleanup/EmoPain_good_run_all.csv', sep='\t', header=None)[0]
    ...
    for n,run_dir in enumerate(run_list):

        run_dir = subj_dir+run_dir+'_events.tsv'
        subj_df_original = pd.read_table(run_dir, sep='\t')
        subj_ID = run_dir.split('/')[-1].split('_')[0]
        run_num = run_dir[-13:-11]
                
        #event file to model CS and US  
        # subj_df_original = pd.read_table('/Users/chendanlei/Dropbox (Partners HealthCare)/U01/EmotionAvoidanceTask/BIDS_BehavioralData/sub-050/sub-050_task-emo3_run-02_events.tsv', sep='\t')
        # subj_df_original = pd.read_table('/Users/chendanlei/Dropbox (Partners HealthCare)/U01/EmotionAvoidanceTask/BIDS_BehavioralData/sub-127/sub-127_task-pain3_run-05_events.tsv', sep='\t')
        ...
        subj_df_event.loc[subj_df_event['trial_type'] == 'CS','trial_type'] = 'CS_combo'
        subj_df_event.loc[subj_df_event['trial_type'] == 'CS_combo','duration'] = [x+y for x, y in zip(subj_df_event['duration'][subj_df_event['trial_type'] == 'CS_combo'].tolist(),subj_df_event['duration'][subj_df_event['trial_type'] == 'decision'].tolist())]
        subj_df_event.loc[subj_df_event['US_trialType']=='negative','trial_type'] = 'USneg'
        subj_df_event.loc[subj_df_event['US_trialType']=='neutral','trial_type'] = 'USneu'    

        subj_df_event = subj_df_event[subj_df_event['trial_type'] != 'decision'] 

        #label each neg and neu US by trial
        #check if this is the first run, if not, add on to the trial number from previous run
        if n == 0:
            subj_df_event.loc[subj_df_event['trial_type']=='USneg','trial_type'] = ['USneg_'+str(i) for i in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneg','trial_type'])))]
            subj_df_event.loc[subj_df_event['trial_type']=='USneu','trial_type'] = ['USneu_'+str(i) for i in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneu','trial_type'])))]
        else:
            #get events from last run
            last_run_event = pd.read_table(os.path.join(subj_dir,subj_ID+'_task-'+os.environ['PROJINIT']+'_'+run_list[n-1].split('_')[-1]+'_events_'+os.environ['PROJNAME']+'.tsv'),sep='\t')
            USnegLast = int([i for i in last_run_event['trial_type'] if 'USneg' in i][-1].split('_')[-1])+1
            USneuLast = int([i for i in last_run_event['trial_type'] if 'USneu' in i][-1].split('_')[-1])+1
            subj_df_event.loc[subj_df_event['trial_type']=='USneg','trial_type'] = ['USneg_'+str(i) for i in [l+USnegLast for l in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneg','trial_type'])))]]
            subj_df_event.loc[subj_df_event['trial_type']=='USneu','trial_type'] = ['USneu_'+str(i) for i in [l+USneuLast for l in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneu','trial_type'])))]]

        subj_df_event=subj_df_event.astype(str)
        subj_df_event[subj_df_event=='nan']='NaN'
        subj_df_event = subj_df_event.reset_index(drop=True)
        # subj_df_event.to_csv(os.path.join('/Users/chendanlei/Desktop/','x'+str(n)+'.tsv'),index=True,sep='\t')
...
########################################
#becuase I'm trying to run this on run level, not the whole subject. Just to make sure the df is correct
run_df_event = pd.read_table(os.path.join(subj_dir,sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2]+'_events_'+os.environ['PROJNAME']+'.tsv'),sep='\t')
print(os.path.join(subj_dir,sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2]+'_events_'+os.environ['PROJNAME']+'.tsv'))
print(sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2])
########################################
...
model_wf.inputs.inputspec.conditions = ['CS_combo']+[i for i in run_df_event['trial_type'] if 'USneg' in i]+[i for i in run_df_event['trial_type'] if 'USneu' in i]
# print('modifying')
# #sub-050_run-01 needs to remove USneg_9 - USneg_11 and USneu_6 - USneu_11
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneg_9']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneg_10']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneg_11']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneu_6']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneu_7']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneu_8']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneu_9']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneu_10']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneu_11']
# #sub-061_run-05 needs to remove USneg_65 - USneg_67 and USneu_51
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneg_65']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneg_66']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneg_67']
# model_wf.inputs.inputspec.conditions = [i for i in model_wf.inputs.inputspec.conditions if i!='USneu_51']
print('conditions:')
print(model_wf.inputs.inputspec.conditions)
model_wf.inputs.inputspec.contrasts = [[i,'T',[i],[1]] for i in model_wf.inputs.inputspec.conditions]
print('contrasts:')
print(model_wf.inputs.inputspec.contrasts)
...
model_wf.inputs.inputspec.bold_template = {'bold': '/scratch/data/'+sys.argv[1]+'/func/'+sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2]+'_bold_space-MNI152NLin2009cAsym_preproc.nii.gz'}
model_wf.inputs.inputspec.mask_template = {'mask': '/scratch/data/'+sys.argv[1]+'/func/'+sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2]+'_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz'}
model_wf.inputs.inputspec.task_template = {'task': '/scratch/data/'+sys.argv[1]+'/func/'+sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2]+'_events_'+os.environ['PROJNAME']+'.tsv'}
model_wf.inputs.inputspec.confound_template = {'confound': '/scratch/data/'+sys.argv[1]+'/func/'+sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2]+'_bold_confounds_temp.tsv'} 
model_wf.inputs.inputspec.smooth_gm_mask_template = {'gm_mask': '/scratch/data/sub-*/anat/'+sys.argv[1]+'_T1w_space-MNI152NLin2009cAsym_class-GM_probtissue.nii.gz'}
...
subject_list = [sys.argv[1]+'_task-'+os.environ['PROJINIT']+'_'+sys.argv[2]]
# subject_list = []
fwhm_list = []
...
# infosource.iterables = [] # If no smoothing.
