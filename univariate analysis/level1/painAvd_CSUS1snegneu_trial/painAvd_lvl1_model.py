...
for subj_dir in subj_list:
    # subj_dir = '/Users/chendanlei/Dropbox (Partners HealthCare)/U01/EmotionAvoidanceTask/BIDS_BehavioralData/sub-055/'
    ...
    for n,run_dir in enumerate(run_list):
        ...
        # subj_df_original = pd.read_table('/Users/chendanlei/Dropbox (Partners HealthCare)/U01/EmotionAvoidanceTask/BIDS_BehavioralData/sub-014/sub-014_task-emo3_run-02_events.tsv', sep='\t')
        ...
        for trial_n in range(len(subj_df_event)):
            # print(trial_n)
            if subj_df_event['trial_type'][trial_n] == 'anticipation':
                #change anticipation row to a copy of US, and change the trial_type and duration
                subj_df_event.loc[trial_n] = subj_df_event.loc[trial_n+1].copy()
                subj_df_event.loc[trial_n,'trial_type'] = 'US_onset'
                subj_df_event.loc[trial_n,'duration'] = 1
                subj_df_event = subj_df_event.reset_index(drop=True)
                subj_df_event.loc[trial_n+1,'trial_type'] = 'US_period'
                subj_df_event.loc[trial_n+1,'onset'] = subj_df_event['onset'][trial_n+1]+subj_df_event['duration'][trial_n]
                subj_df_event.loc[trial_n+1,'duration'] = 2
                subj_df_event = subj_df_event.reset_index(drop=True)
        subj_df_event = subj_df_event.reset_index(drop=True)
        ...
        if n == 0:
            subj_df_event.loc[subj_df_event['trial_type']=='USneg_onset','trial_type'] = ['USneg_onset'+str(i) for i in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneg_onset','trial_type'])))]
            subj_df_event.loc[subj_df_event['trial_type']=='USneu_onset','trial_type'] = ['USneu_onset'+str(i) for i in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneu_onset','trial_type'])))]
        else:
            #get events from last run
            # last_run_event = pd.read_table(os.path.join(subj_dir,'x'+str(n-1)+'.tsv'),sep='\t')
            ...
            USnegLast = int([i for i in last_run_event['trial_type'] if 'USneg_onset' in i][-1].split('_onset')[-1])+1
            USneuLast = int([i for i in last_run_event['trial_type'] if 'USneu_onset' in i][-1].split('_onset')[-1])+1
            subj_df_event.loc[subj_df_event['trial_type']=='USneg_onset','trial_type'] = ['USneg_onset'+str(i) for i in [l+USnegLast for l in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneg_onset','trial_type'])))]]
            subj_df_event.loc[subj_df_event['trial_type']=='USneu_onset','trial_type'] = ['USneu_onset'+str(i) for i in [l+USneuLast for l in list(range(len(subj_df_event.loc[subj_df_event['trial_type']=='USneu_onset','trial_type'])))]]
...
model_wf.inputs.inputspec.conditions = ['CS_combo','USneg_period','USneu_period']+[i for i in run_df_event['trial_type'] if 'USneg_onset' in i]+[i for i in run_df_event['trial_type'] if 'USneu_onset' in i]
# ####################
# print('modifying')
# #sub-050_run-04 needs to remove anything after USneg_onset32
# #sub-056_run-05 needs to remove anything after USneg_onset57
# #sub-072_run-01 needs to remove anything after USneg_onset4
# #sub-091_run-02 needs to remove anything after USneg_onset15
# #sub-070_run-04 needs to remove anything after USneg_onset24
# #sub-090_run-05 needs to remove anything after USneg_onset13
# first_element_to_delete = np.where(np.array(model_wf.inputs.inputspec.conditions)=='USneg_onset13')[0][0] #then delete all elements after that
# print(first_element_to_delete)
# del model_wf.inputs.inputspec.conditions[first_element_to_delete:]
# ####################
...
model_wf.inputs.inputspec.bold_template = {'bold': '/scratch/data/sub-*/func/sub-*_task-'+os.environ['PROJINIT']+'_run-*_bold_space-MNI152NLin2009cAsym_preproc.nii.gz'}
model_wf.inputs.inputspec.mask_template = {'mask': '/scratch/data/sub-*/func/sub-*_task-'+os.environ['PROJINIT']+'_run-*_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz'}
model_wf.inputs.inputspec.task_template = {'task': '/scratch/data/sub-*/func/sub-*_task-'+os.environ['PROJINIT']+'_run-*' + '_events_'+os.environ['PROJNAME']+'.tsv'}
...
model_wf.inputs.inputspec.confound_template = {'confound': '/scratch/data/sub-*/func/sub-*_task-'+os.environ['PROJINIT']+'_run-*_bold_confounds_temp.tsv'} 
